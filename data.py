...
class PCGDataset(Dataset):
    def __init__(self, 
        ...
        self.patient_files, self.recordings, self.grades= load_recordings_with_labels(self.data_folder, self.classes)
    ...
    def __getitem__(self, idx):
        ...
        if self.target == 'Systolic murmur grading':
            return spec, patient_features, grade
...
class Preprocessor:
    ...
    def measure_grade(self, spectrogram, low=20, high=200):
        split_start = round(2 * 224 / self.frequency * low)
        split_end = round(2 * 224 / self.frequency * high)
        grade = spectrogram[split_start:split_end, :].sum() / spectrogram.sum()
        return grade
    ...
    def __call__(self, recording, freq, interval=1.0):
        ...
        if self.transform:
            ...
            if self.mode == 'test':
                ...
                ratio = np.array([self.measure_grade(multi_scale_specs[0][i][0]) for i in range(multi_scale_specs[0].shape[0])])
            else:
                ratio = self.measure_grade(multi_scale_specs[0][0])
