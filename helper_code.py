...
# Get grade status from patient data.
def get_grade(data):
    grade = next((line.split(': ')[1].strip() for line in data.split('\n') if line.startswith('#Systolic murmur grading:')), None)
    if grade is None:
        raise ValueError('No grade available. Is your code trying to load labels from the hidden data?')
    return grade
...
def save_challenge_outputs(filename, patient_id, classes, labels, probabilities):
    ...
    expected_classes = ["I/VI","II/VI","III/VI"]
